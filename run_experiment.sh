...
MODEL_NAME=${2:-mobilenetv2_split_size_1_lpt}   # default model
...
tmux send-keys -t $SESSION:dispatcher 'conda activate zk-torch' C-m
...
for i in $(seq 1 $NUM_WORKERS); do
    ...
    tmux send-keys -t $SESSION:worker$i 'conda activate zk-torch' C-m
...
tmux send-keys -t $SESSION:submit_job 'conda activate zk-torch' C-m
