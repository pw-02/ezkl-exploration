...
class ZKPWorkerServicer(pb2_grpc.ZKPWorkerServiceServicer):
    def __init__(self):
        ...
        self.requests = {}  # Store request status
    ...
    def process_request(self, request_id, request):
            ...
            prover = EZKLProver(working_dir, self.log_dir, orverwrite=False)
            try:
                ...
                logging.error(f"Failed to compute proof for request {request_id}. {str(e)}")
                with self.lock:
                    self.requests[request_id] = {
                        'status': 'Completed',
                        'success': False,
