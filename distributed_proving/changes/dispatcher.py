...
class ZKPProver():
    ...
    def compute_proof_for_model(self, model: OnnxModel, worker: Worker):
        ...
        try:
            ...
            while True:
                try:
                    proof_status = stub.CheckProofStatus(pb2.ProofStatusRequest(request_id=request_id), timeout=30)
                    if proof_status.is_completed:
                        model.isprocessed = True
                        if proof_status.is_successful:
                            model.computed_proof = proof_status.proof
                        ...
                        performance_metrics = json.loads(proof_status.performance_metrics)
                        break
                    else:
                    ...
                    stub = pb2_grpc.ZKPWorkerServiceStub(channel)
                    continue
            self.write_report(worker.address, model.model_info, proof_status.is_successful, performance_metrics)
        except Exception as e:
