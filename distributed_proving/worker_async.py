...
class EZKLProver:
    ...
    async def run_end_to_end_proof(self):
        with ResourceMonitor() as monitor:
            ...
            for func_name, func in functions:
                execution_time = await asyncio.to_thread(func)
...
class ZKPWorkerServicer(pb2_grpc.ZKPWorkerServiceServicer):
    ...
    async def ComputeProof(self, request, context):
    ...
    async def CheckProofStatus(self, request, context):
    ...
    async def process_request(self, request_id, request):
        logging.info("Processing 'Compute Proof' request.")
        ...
        prover = EZKLProver(directory_path, self.log_dir, overwrite=False)
        proof_path, performance_data = await asyncio.to_thread(prover.run_end_to_end_proof)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='ZKP Worker Service')
