...
def find_closest_power(total_value):
    ...
    return math.ceil(math.log2(total_value))
...
if __name__ == "__main__":
    ...
    MODEL_INPUT_SHAPE_MAP = {
        'resnet18.onnx': '{"input": [1, 3, 224, 224]}',
        'resnet50.onnx': '{"input": [1, 3, 224, 224]}',
        'resnet101.onnx': '{"input": [1, 3, 224, 224]}',
        'resnet152.onnx': '{"input": [1, 3, 224, 224]}',
        'mnist_classifier.onnx': '{"input": [1, 1, 28, 28]}'
    }

    ONNX_MODEL_PATH = '/home/pw/projects/zkinfer/examples/onnx/mnist_classifier/network.onnx'
    SIMPLIEFIED_MODEL_PATH = '/home/pw/projects/zkinfer/examples/onnx/mnist_classifier/network_simplified.onnx'
    UPPER_BOUND_PER_SUBGRAPH = 8
    INPUT_SHAPE = MODEL_INPUT_SHAPE_MAP.get('mnist_classifier.onnx', '{}')
    SIMPLIFY_FLAG = True




    parser.add_argument("--onnx_model_path", type=str, default=ONNX_MODEL_PATH, help="Path to the ONNX model. Default is './resnet18.onnx'.")
    parser.add_argument("--simplified_model_path", type=str, default=SIMPLIEFIED_MODEL_PATH, help="Path to save the simplified ONNX model. Default is './resnet18_simplified.onnx'.")
    parser.add_argument("--upper_bound_per_subgraph", type=int, default=UPPER_BOUND_PER_SUBGRAPH, help="Upper bound per subgraph in the form of 2^n. Default is 23.")
    parser.add_argument("--input_shape", type=str, default=INPUT_SHAPE, 
                        help="Input shape for the ONNX model in JSON format. Default is '{\"input\": [1, 3, 224, 224]}'.")
    parser.add_argument("--simplify", action='store_true', 
                        default=SIMPLIFY_FLAG,
